...
namespace mgl::opengl
{
  ...
  buffer_layout::buffer_layout(const std::string& layout)
      : m_layout(layout)
  {
    ...
    for(buffer_layout::iterator it = begin(); it != end(); ++it)
    {
      ...
      m_layout_info.size += it.value().size;
      m_layout_info.nodes += it.value().type ? 1 : 0;
    }
  ...
  buffer_layout::iterator::iterator(const char* ptr)
      : m_ptr(ptr)
  {
    if(ptr == nullptr)
    {
      m_element = { 0, 0, 0, false };
      return;
    }
    ...
    while(true)
    {
      ...
      switch(chr)
      {
        ...
        default: m_element = { 0, 0, 0, false }; return;
      }
