...
namespace mgl::metal
{
  ...
  {
public:
    static context_ref create_context(context_mode::mode mode);

    context(context_mode::mode mode);
    ~context();
    ...
    metal_context device() const { return m_device; }

    metal_context command_queue() const { return m_commandQueue; }
...
private:
  ...
  {
    return context::create_context(mode);
  }
