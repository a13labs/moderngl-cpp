...
public:
    ...
    virtual device::dialect api() const override final { return device::dialect::OPENGL; }

    static mgl::opengl::context_ref& current_context()
    {
      auto& api = mgl::platform::device::instance();
      MGL_CORE_ASSERT(api.api() == device::dialect::OPENGL, "Invalid API");
