...
{
  template <typename T>
  class metal_buffer : public T
  {
public:
    metal_buffer(size_t size, bool dynamic)
        : m_buffer(nullptr)
        , m_req_size(size)
        , m_req_dynamic(dynamic)
    { }

    virtual ~metal_buffer() = default;

    virtual void allocate() override
    {
      MGL_CORE_ASSERT(!m_buffer, "Buffer is already allocated");
      auto& ctx = metal_api::current_context();
    ...
    mgl::metal::buffer_ref& native() { return m_buffer; }

protected:
    mgl::metal::buffer_ref m_buffer;
...
private:
  ...
  class metal_generic_buffer : public metal_buffer<mgl::platform::api::buffer>
  {
public:
    metal_generic_buffer(size_t size = 0, bool dynamic = false)
        : metal_buffer(size, dynamic)
    { }

    virtual ~metal_generic_buffer() = default;
  };

  class metal_vertex_buffer : public metal_buffer<mgl::platform::api::vertex_buffer>
  {
public:
    metal_vertex_buffer(const std::string& layout,
    ...
    virtual ~metal_vertex_buffer() = default;

    virtual void allocate() override final
    {
      metal_buffer::allocate();
    ...
    const mgl::metal::vertex_buffer& native_vbo() const { return m_vbo; }

private:
    mgl::metal::vertex_buffer m_vbo;
  };

  class metal_index_buffer : public metal_buffer<mgl::platform::api::index_buffer>
  {
public:
    metal_index_buffer(size_t size = 0, uint16_t element_size = 4, bool dynamic = false)
        : metal_buffer(size, dynamic)
        , m_element_size(element_size)
    { }

    virtual ~metal_index_buffer() = default;
...
private:
  ...
  using metal_vertex_buffer_ref = mgl::ref<metal_vertex_buffer>;
  using metal_index_buffer_ref = mgl::ref<metal_index_buffer>;
