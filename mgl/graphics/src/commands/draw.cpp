...
namespace mgl::graphics
{
  void draw_command::execute()
  {
    if(m_index_buffer)
    {
      mgl::platform::api::geom_data geom = { m_vertex_buffer->api(),
                                           m_vertex_buffer->layout(),
                                           m_index_buffer->api(),
                                           m_index_buffer->element_size(),
                                           m_mode };
      geom.allocate();
      mgl::platform::api::draw(geom, m_model_view, m_count, m_offset, m_instance_count);
      geom.deallocate();
    }
    else
    {
  ...
  void draw_batch_command::execute()
  {
    ...
    {
      mgl::platform::api::geom_data geom = { m_batch->vertex_buffer()->api(),
                                           m_batch->vertex_buffer()->layout(),
                                           m_batch->index_buffer()->api(),
                                           m_batch->index_buffer()->element_size(),
                                           m_batch->mode() };
      geom.allocate();
      mgl::platform::api::draw_batch(geom, m_batch->get());
      geom.deallocate();
    }
    else
    {
